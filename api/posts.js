  ...
  if (!slug) {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        const now = Date.now();
        const post = { slug: s, title: escapeHtml(title), excerpt: escapeHtml(excerpt || content.slice(0, 160)), category: escapeHtml(category), content, coverImage, readTime: estimateReadTime(content), createdAt: now };
